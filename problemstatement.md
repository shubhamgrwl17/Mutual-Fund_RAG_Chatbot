...
Overview
The objective of this project is to build a facts-only FAQ assistant for mutual fund schemes, using Groww mutual fund scheme pages as the data source. The assistant will answer objective, verifiable queries related to mutual funds by retrieving information from a curated corpus built exclusively from the Groww scheme pages listed in the project architecture.
...
Scope of Work
1. Corpus Definition
Select 6 mutual fund schemes (Mid-Cap category) across multiple AMCs, as specified in `docs/rag_architecture.md`.
Collect the 6 Groww scheme page URLs as the corpus. No PDFs are used; data is extracted from each page’s embedded `__NEXT_DATA__` JSON.
...
Constraints
Data and Sources
Use only the curated Groww scheme page URLs specified in `docs/rag_architecture.md`
Do not use third-party blogs beyond the approved Groww corpus URLs
